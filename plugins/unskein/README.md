...
**역할을 몰라도 시작할 수 있다** — 관리자에게 받은 토큰만 있으면 `unskein-setup` 이 서버로 토큰 종류(kind)를 판별해(R0, ADR-0027) 알맞은 역할 절차로 분기하고, 설치할 프로젝트도 멤버십 목록에서 고르게 하며, 작업 디렉토리도 이름을 물어 셋업이 만든다. 토큰 값은 대화·셸 인자에 넣지 말고 `~/.unskein/setup.env` 에 편집기로 넣는다 — 스킬 호출 위치는 무관하다.
