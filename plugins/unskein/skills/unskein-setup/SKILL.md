...
- **프로젝트 격리**: 한 distro 에서 여러 프로젝트 → **프로젝트 디렉토리에서 이 스킬을 실행**하면 `$PWD/.unskein` 을 `UNSKEIN_HOME` 으로 기본 제안한다. 확정 값은 **절대경로**로 executor.env **맨 위에** `export UNSKEIN_HOME=…` 로 기록한다(자기완결 — 이후 어느 cwd 에서든 `source <프로젝트>/.unskein/executor.env` 그 파일 하나로 creds·work 까지 전부 확정. `~` 축약·상대경로로 적지 않는다).
- **운용 전제(프로젝트 격리)**:
