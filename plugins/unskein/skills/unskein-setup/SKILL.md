...
〔검사: `~/.unskein/executor.env` 에 API·모리토큰·git 토큰이 있고 `/api/health` 200 · `/api/mori/scope` 200 이면 이 단계 스킵〕

1. **파악**: 타겟 서버(`UNSKEIN_API`) · 이 실행기가 맡을 **프로젝트 하나**(비즈니스+프로젝트 이름 = watch scope) · 그 프로젝트 repo 주소·스택(node/python 등)·멤버십.
2. **`~/.unskein/executor.env` 작성**(chmod 600) — 템플릿 `${CLAUDE_PLUGIN_ROOT}/templates/executor.env.sample` 복사 후 **편집기로** 채운다(값은 export, 셸 인자로 넣지 말 것):
   - 서버: `UNSKEIN_API` + `UNSKEIN_MORI_TOKEN`(**kind=mori** — ⚠️ planner 아님, ADR-0013).
   - git(HTTPS): `UNSKEIN_GIT_TOKEN`(clone/push) + `GH_TOKEN`(gh pr create, 같은 PAT 재사용 가능). / (SSH): 키 파일은 `~/.unskein/creds/id_ed25519`(600), 경로는 `UNSKEIN_SSH_KEY`.
...
3. **자동 로드 한 줄**을 `~/.bashrc` 에 심는다(매 셸 자동 로드 → 이후 수동 `source` 불요):
   `set -a; [ -f ~/.unskein/executor.env ] && . ~/.unskein/executor.env; set +a`
4. **환경 점검**: WSL 안 `claude`/`git`/`gh` 존재·인증(gh 는 `GH_TOKEN` env 또는 `gh auth login`) · `~/.unskein/creds`·`~/.unskein/work` 폴더 · `/api/health` 도달 + `/api/mori/scope` 200(토큰 유효). 없거나 미설정이면 설치·설정을 안내하고 멈춘다.
...
2. **실클론**: 그 repo 를 자격증명으로 작업 루트(`UNSKEIN_WORK_ROOT`, 기본 `~/.unskein/work`)에 **실제 클론**한다(env/askpass·SSH — 토큰 무잔존). `ls-remote` 로 그치지 않는다 — 토큰·키가 end-to-end 로 도는지 확정. (이후 모리 `prepare_repo` 가 이 클론을 fetch·리셋으로 재사용.)
