...
> **무repo 프로젝트**(프로젝트에 repo 주소 미등록 — 코드베이스 없이 사이트 API 를 다루는 작업형): 아래 2(실클론)·3(의존성)은 대상이 없어 건너뛴다. 실행기는 그런 프로젝트의 **사용자 프로세스 카드를 클론 없이** 작업 폴더만으로 수행한다(v1.46.0+). 코드 개발(dev) 카드는 여전히 repo 필수다. ⚠️ S1 의 git 토큰·gh 인증은 무repo 여도 아직 필요하다 — preflight 가 치명 항목으로 검사한다(무repo 프로젝트용 완화는 후속).
...
- **경로 2 — 비즈니스별 토큰/사이트별 로그인(병렬·격리)**: 프로젝트마다 {토큰·API·WATCH, CDP 포트+프로필, cases}를 **격리한 번들**로 나눈다. 동시 병렬 검증·사이트별 로그인 분리에 필요. 아래 **T1–T4** 로 프로비저닝하고 CronCreate `*/5` 를 **config 마다** 건다.
...
### T3. 단계 스킬 환경값 — 담당 스킬이 요구하는 값 채우기 (경로 1·2 공통)

앞 절의 값(큐 왕복·CDP)만으로는 **카드를 받을 뿐**이다. 검증을 실제로 수행하는 것은 그 단계에 배정된 **단계 스킬**이고, 그 스킬이 자기 `입력` 절에 요구 환경값을 선언한다. 번들에 그 값이 없으면 스킬은 절차 1에서 **진행하지 않고 QUESTION** 을 낸다(조용한 우회 금지) — 카드는 선점된 채 waiting 으로 넘어가고 사람이 답해야 풀린다. 그래서 **claim 을 돌리기 전에** 채운다.
...
현재 실물 — `frame9_bnd` 검수(`frame9bnd-step06-verify-cdp`)를 담당하는 번들이 요구하는 값:
...
### T4. 실행 표준 (두 경로 — CronCreate 는 config 단위)
