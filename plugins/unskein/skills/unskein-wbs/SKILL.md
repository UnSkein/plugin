...
### 4.6 화면검증(CDP) 카드 — 표식·주소·검증 정보

화면이 바뀌는 카드는 개발만으로 끝나지 않는다. 실행기(다오)는 헤드리스 WSL 이라 브라우저를 띄울 수 없고, 화면 확인은 **TESTER**(윈도우 CDP 단말)가 별도 카드로 맡는다. 이 절은 **그 카드가 일할 수 있게 등록 때 갖춰야 할 것**을 정한다. 단일 출처: `docs/specs/cdp-verify-standard-stage.md`.

**왜 등록 때인가**: 검증 카드는 원본 카드가 완료될 때 서버가 자동으로 만들고, 그 지시문을 **원본의 `plan_doc`·`result_doc` 에서 합성**한다(SaaS `_cdp_request_plan_doc`). 즉 **여기서 넣은 완료 기준이 그대로 TESTER 의 검증 기준이 된다.** 등록 때 부실하면 검증 때 메울 방법이 없다 — 2026-07-30 실측에서 의뢰 카드 15건 중 14건의 지시문이 비어 있었고, 한 검증(#783)은 "기대값 표가 비공개 repo 에 있어 읽을 수 없다"를 P1 결함으로 냈다.

**① 표식을 켠다 — `payload.needs_cdp=true`**
화면이 바뀌는 말단 카드에 이 표식을 켜면 그 카드가 완료될 때 검증 카드가 자동 생성된다. 등록 때 `POST /api/tasks` 의 `payload` 에 함께 넣는다(웹 보드의 토글 버튼과 같은 자리).

```json
{"needs_cdp": true}
```

- 화면이 없는 코드 전용 카드(백엔드·스키마·문서)에는 켜지 않는다. 프로젝트에 `settings.require_cdp=true` 가 켜져 있으면 표식 없이도 전부 생성되므로, 그런 프로젝트에서 화면 없는 카드는 `payload.no_cdp=true` 로 뺀다.
- **검증 끝배치**: 한 기능의 화면 카드가 여러 장이면 표식을 마지막 카드에만 켠다 — 중간마다 검증 카드가 생기면 미완성 화면을 검증하게 된다.

**② 완료 기준을 화면 문장으로 쓴다**
§4.1 의 **[완료 기준]** 이 검증 카드로 그대로 넘어간다. **"무엇이 화면에 보이면 통과인가"** 로 적는다.

- O: "설정 화면에서 토큰 설명을 인라인으로 고칠 수 있고, 저장 후 목록에 바뀐 설명이 보인다"
- X: "설명 편집 기능 구현" (화면에서 판정할 수 없다)
- X: "기대값은 사내 repo `spec/erp-fields.md` 참조" (**TESTER 는 그 repo 에 못 닿는다** — 기대값은 카드 안에 값으로 적거나, 검증자가 열 수 있는 화면·API 를 출처로 지목한다)

**③ 화면 크기는 기본값을 쓴다 — 카드에 적지 않는다**
검증은 **1920×1080** 기준으로 돌아간다(검증기 스킬 `unskein-test` §0.6 이 강제). 카드마다 적으면 반복되고 한 장만 빠뜨려도 그 검증은 재현되지 않으므로 **기본값은 적지 않는다.** 적는 것은 예외뿐이다 — 잘림·반응형 레이아웃이 쟁점이면 [완료 기준]에 "보조 크기 1366×768 로도 확인한다"를 명시한다(그 크기의 결과는 따로 적히게 된다).

**④ [실행 정보]에 검증 진입 정보를 적는다**
화면 카드의 [실행 정보]는 개발자용 파일 경로에서 끝내지 않고 **검증자가 화면에 도달할 경로**를 함께 적는다 — 화면 경로(예 `/settings`), 로그인 방법(계정 **값**이 아니라 보관 위치 — §4.4), 확인할 시나리오. 시나리오를 구조화해 넘기려면 `payload.scenarios` 에 배열로 넣는다(검증 카드 지시문에 불릿으로 펼쳐진다).

**⑤ 프로젝트 검증 주소를 먼저 확인한다 — `project.settings.tested_url`**
검증 카드의 주소는 **카드 값 > 프로젝트 기본값** 순으로 풀린다. 보드의 의뢰 버튼은 주소를 입력받지 않으므로 **프로젝트 기본값이 사실상 유일한 공급원**이다. 등록 전에 확인하고 비어 있으면 채운다(웹: 비즈니스 화면 → 프로젝트 카드 → "화면검증 주소", API: `PATCH /api/projects/{id}` 의 `settings`).

```
GET /api/businesses/{bid}/projects  →  settings.tested_url 확인
PATCH /api/projects/{id}  {"settings": {…기존 키…, "tested_url": "https://…"}}
```
`settings` 는 **통째 교체**다 — 기존 키(`slug` 등)를 펼쳐 병합해 보낸다. 비울 때는 빈 문자열이 아니라 키를 지운다(빈 문자열은 "설정됨"으로 오인된다).

**⑥ 검증 카드를 직접 등록해야 할 때**
이미 완료된 화면을 다시 검증하는 등, 원본 완료를 거치지 않고 검증 카드만 필요하면:

- **1차 경로** — 완료 카드에서 의뢰: `POST /api/tasks/{원본id}/cdp-request` (`tested_url`·`scenarios`·`note` 선택). 마커·부모 앵커·지시문 합성을 서버가 붙인다. **dev 프로세스 카드에서만** 되고 비-dev 는 409.
- **직접 만들기(2단계)** — dev 카드는 **등록(backlog)으로만 생성된다**(실행대기 이상은 계획·파이프라인 경유 — 409). 그래서 ① `POST /api/tasks` 로 backlog 생성(`payload={"cdp_request": true, "tested_url": "…"}`) → ② `PATCH /api/tasks/{id}` 로 `status="test"`. **`cdp_request` 마커를 빠뜨리지 않는다** — 없으면 합격 후 인스팩션으로 넘어가 실행기가 커밋할 것도 없이 마감을 시도한다.
- 어느 경로든 `plan_doc` 을 §4.1 로 채운다. 검증 카드에도 자기완결이 그대로 요구된다.

**⑦ 검증을 별도 카드로 뗄지 판단**
§4.1 의 실행 주체 규칙이 여기에 걸린다 — 실행기가 못 닿는 자원(브라우저 화면·라이브 배포본)을 완료 기준으로 요구하는 말단은 실행기 카드에 두지 않는다. 화면 확인이 완료 기준의 본체라면 표식(①)으로 검증 카드를 파생시키고, 실행기 카드의 완료 기준은 코드·빌드·타입체크로 닫는다.
...
3. 각 노드: 이미 같은 `wbs_code`가 있으면 skip(멱등). 없으면 `parent_id` = 부모 wbs_code의 id, `sort_order` = 증가값으로 생성. **말단이면 §4 자기완결 표준(7요소)으로 `plan_doc`을 채우고, 루트면 스콥 전문을 싣는다. `plan_start`/`plan_end`는 §6(오늘 기준·의존성 계단식)으로 계산해 함께 넣는다** — 날짜를 비우면 간트에서 오늘 열에 몰리므로 빈 날짜 금지.
   - **화면이 바뀌는 말단이면 §4.6 을 함께 적용한다** — `payload.needs_cdp` 표식(검증 끝배치), 화면에서 판정 가능한 완료 기준, [실행 정보]의 검증 진입 경로. 프로젝트 `settings.tested_url` 이 비어 있으면 등록 전에 채운다(검증 카드가 주소 없이 생성된다).
...
- **화면검증(§4.6)**: 화면이 바뀌는 말단에 `payload.needs_cdp` 가 켜졌는지(한 기능에 하나 — 끝배치) · 그 카드의 완료 기준이 화면에서 판정 가능한 문장인지 · 기대값 출처가 TESTER 가 닿을 수 있는 곳인지(비공개 repo 지목 금지) · 프로젝트 `settings.tested_url` 이 채워졌는지 본다.
