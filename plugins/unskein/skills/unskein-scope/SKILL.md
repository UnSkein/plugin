...
- **인증은 플래너 토큰**(kind=planner) — 서버 호출 전에 인증 로드로 셸에 올린다(프로젝트별 격리 `planner.env`, source 우선·cwd 폴백 — ADR-0021):
  ```bash
  . "${CLAUDE_PLUGIN_ROOT}/bin/planner-env.sh"   # UNSKEIN_API + UNSKEIN_PLANNER_TOKEN 로드
  ```
  이후 후보를 받아 `repo_url` 일치 프로젝트를 찾는다: `GET /api/businesses` → `GET /api/businesses/{biz}/projects`(둘 다 `X-Planner-Token: $UNSKEIN_PLANNER_TOKEN` — get_current_user_flex 인가, ADR-0013) → 각 프로젝트 `repo_url` 과 비교. **표기 차이는 정규화**해 맞춘다(`.git` 접미 제거, `https://`↔`git@`(ssh) 호스트/경로 동치, 대소문자). (사람이 웹 로그인 세션으로 직접 돌리면 그 Bearer 로도 같은 유저로 인가된다 — `POST /api/auth/login`. **토큰이 안 잡히면 멈추고 알린다**.)
