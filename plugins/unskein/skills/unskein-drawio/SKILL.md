...
- **신분(토큰)**: `UNSKEIN_PLANNER_TOKEN`(kind=planner) + `UNSKEIN_API`. 이 둘은 **프로젝트별로 격리된 `planner.env`** 에 있고, 서버 호출 전에 **인증 로드**로 셸에 올린다(아래):
  ```bash
  . "${CLAUDE_PLUGIN_ROOT}/bin/planner-env.sh"   # planner.env 로드 — source 우선·cwd 폴백(ADR-0021)
  ```
  해석 순서: ① 이미 셸 env 에 있으면 그대로(명시적 `source` 존중) → ② `$UNSKEIN_HOME/planner.env` → ③ (폴백) cwd 에서 위로 첫 `.unskein/planner.env`(지금 이 프로젝트 디렉토리 자동인식) → ④ 전역 `~/.unskein/planner.env`. 한 플래너로 여러 프로젝트를 다룰 때 프로젝트마다 토큰·서버가 안 섞인다. **토큰이 안 잡히면 멈추고 알린다(fallback 금지).**
...
목록 조회(플래너 토큰):
```bash
. "${CLAUDE_PLUGIN_ROOT}/bin/planner-env.sh"   # planner.env 로드(§1 — source 우선·cwd 폴백)
