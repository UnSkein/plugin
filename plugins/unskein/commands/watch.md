...
> **어느 서버를 보는지는 환경의 `UNSKEIN_API`+`UNSKEIN_MORI_TOKEN` 짝이 정한다** — 토큰은 그 서버 전용이라 안 맞으면 401. 로컬을 보려면 로컬 API+토큰, 테스트 서버를 보려면 그 서버의 API+토큰. 설정은 `~/.unskein/executor.env`(export 라 source 하면 자식에 상속) 하나에 담고 watch 전에 `source` 한다. 여러 서버를 오가면(드묾) `executor.local.env` 등으로 파일만 나눠 그 하나만 source(짝 분리 — `unskein-setup` S1).
